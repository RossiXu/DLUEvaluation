...
BEST_DEV_LOSS = TEST_LOSS = -10000000
...
def run(e):
    global BEST_DEV_LOSS, TEST_LOSS, LOSS_INCREASE_TIME
    ...
    train_batch = data_utils.Minibatcher(
        ...
        if_lf=e.config.model_type in ["longformer","xlnet","linformer"],
        verbose=True,
        pad=(e.config.max_tot_src_len,e.config.max_entry) if e.config.model_type=="linformer" else None)
    ...
    elif e.config.model_type == 'xlnet':
        model_type = models.XLNetSeq2Seq
    elif e.config.model_type == 'linformer':
        model_type = models.LinformerSeq2Seq
    else:
    ...
    dev_eval = train_helper.Evaluator(
        ...
        vocab=data.vocab,
        if_lm=False,
        if_lf=e.config.model_type in ["longformer", "basic","xlnet","linformer"],
    ...
    for epoch in range(start_epoch, e.config.n_epoch):
        for it, (input_data, input_attn_mask, global_attn_mask, tgt_inp, tgt_mask,
            ...
            if e.config.auto_disconnect and full_division:
                if e.elapsed_time > 99999999999993.5:
            ...
            if (true_it % e.config.print_every == 0 or
                ...
                summarization = train_stats.summarize(
                    "epoch: {}, it: {} (max: {}), lr: {:.4e}"
                        .format(epoch, true_it % (len(train_batch.idx_pool) // e.config.gcs),
                                len(train_batch.idx_pool) // e.config.gcs, curr_lr))
            ...
            if (true_it % e.config.eval_every == 0 or
                ...
                e.log.info("best dev loss: {:.4f}, test loss: {:.4f}, loss increase time: {:d}"
                           .format(BEST_DEV_LOSS, TEST_LOSS, LOSS_INCREASE_TIME))
                train_stats.reset()

                if LOSS_INCREASE_TIME > 10 and e.config.stop_early:
                    e.log.info(
                        f"stop training because dev loss increased for {LOSS_INCREASE_TIME} times")
                    train_batch.save()
                    model.save(
                        dev_bleu=BEST_DEV_LOSS,
                        test_bleu=TEST_LOSS,
