...
def run(e):
    ...
    eval_batch = data_utils.Minibatcher(
        ...
        mask_fraction=0.0,
        random_perm=False,
        if_lf=model_exp.config.model_type in ["longformer","xlnet","linformer"],
        is_eval=True,
        verbose=True,
        pad=(model_exp.config.max_tot_src_len,model_exp.config.max_entry) if model_exp.config.model_type=="linformer" else None)
    ...
    elif model_exp.config.model_type == "xlnet":
        model_type = models.XLNetSeq2Seq
    elif model_exp.config.model_type == "linformer":
        ...
        for nbatch, (input_data, input_attn_mask, global_attn_mask, tgt_inp, tgt_mask,
            ...
            for out_id, idx in zip(out_ids, batch_idx):
                ...
                if e.config.debug:
                    ...
                    e.log.info("**** REFERENCE: {}".format(data.test_data[idx]["tgt_tok"]))
                    e.log.info("************")
    ...
    with open(file_name, "w") as fp, open(ref_file_name, "w") as ref_fp:
        for gen in sorted(all_gen.items(), key=lambda x: x[0]):
            fp.write(gen[1] + "\n")
            ref_fp.write(data.test_data[gen[0]]["tgt_tok"] + "\n")
