...
from transformers import LongformerConfig, LongformerModel, XLNetConfig, XLNetModel
...
class LongformerSeq2Seq(Base):
    ...
    def forward(self, input_data, input_attn_mask, global_attn_mask, tgt_inp, tgt_mask, tgt_tgt, eot_idx, eot_mask):
        ...
        # eot_mask是取出来的句子的EOS位置，的mask，因为每条数据的EOS数目不一样
...
class XLNetSeq2Seq(Base):
    @auto_init_pytorch
    @auto_init_args
    def __init__(self, vocab_size, iter_per_epoch, experiment):
        super(XLNetSeq2Seq, self).__init__(iter_per_epoch, experiment)
        self.config = {
              "architectures": [
                "XLNetLMHeadModel"
              ],
        ...
        self.config["vocab_size"] = vocab_size
        self.config["n_layer"] = self.expe.config.elayer
        self.config["d_model"] = self.expe.config.hsize
        # self.config["attention_window"] = [512] * self.expe.config.elayer # longformer专有参数
        self.config["d_inner"] = self.expe.config.hsize * self.expe.config.fns
        self.config["n_head"] = 8
        xlconfig = XLNetConfig(**self.config)
        xlconfig.gradient_checkpointing = self.expe.config.gradient_checkpointing
        self.encoder = XLNetModel(xlconfig)
    ...
    def forward(self, input_data, input_attn_mask, global_attn_mask, tgt_inp, tgt_mask, tgt_tgt, eot_idx, eot_mask):
        # 把所有tensor放到device
        input_data, input_attn_mask, tgt_inp, tgt_mask, tgt_tgt, eot_idx, eot_mask = \
            self.to_tensors(input_data, input_attn_mask, tgt_inp, tgt_mask, tgt_tgt, eot_idx, eot_mask)
        # print(input_data.shape)
        data_vec = self.encoder(input_ids=input_data.long(), attention_mask=input_attn_mask)[0]
        data_vec = data_vec[eot_idx.bool()]
...
class LinformerSeq2Seq(Base):
    @auto_init_pytorch
    @auto_init_args
    def __init__(self, vocab_size, iter_per_epoch, experiment):
        super(LinformerSeq2Seq, self).__init__(iter_per_epoch, experiment)
        self.config = {
            "vocab_size": 30522,
            "hidden_size": 768,
            "num_hidden_layers": 12,
        ...
        self.config["vocab_size"] = vocab_size
        self.config["seq_length"] = self.expe.config.max_tot_src_len + self.expe.config.max_entry
        self.config["num_hidden_layers"] = self.expe.config.elayer
        self.config["hidden_size"] = self.expe.config.hsize
        # self.config["attention_window"] = [512] * self.expe.config.elayer
        self.config["intermediate_size"] = self.expe.config.hsize * self.expe.config.fns
        self.config["num_attention_heads"] = 8
        lfconfig = LinformerConfig(**self.config)
        # lfconfig.gradient_checkpointing = self.expe.config.gradient_checkpointing
        self.encoder = LinformerModel(lfconfig)
