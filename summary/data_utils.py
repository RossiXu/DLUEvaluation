...
class DataProcessor:
    ...
    def process(self):
        ...
        test_data = self._load_instance(
            self.test_path, vocab, True,
            max_src_len=self.expe.config.max_src_len,
            max_tgt_len=self.expe.config.max_tgt_len)
    ...
    # path 示例：/shared_home/guanxinyan2022/data/SummScreen/train
    def _load_instance(self, path, vocab, is_test,
        ...
        tgt_len = []
        src_per_len = []
        src_n_entry = []  # 存储数据输入长度
        src_n_entity = []
        tgt_n_entity = []
        src_unique_ent = []
        tgt_unique_ent = []
        fp_doc = open(path + '.doc.json')
        fp_ann = open(path + '.ann.json')
        # 文件为json文件，每行是一条数据
        doc = json.load(fp_doc)
        ann = json.load(fp_ann)
        fp_doc.close()
        fp_ann.close()

        for nline, line in enumerate(doc):
            # if nline>500:
            #     break
            if line["id"] != ann[nline]["id"]:
                raise f"id{line['id']} in doc not match id {ann[nline]['id']} in ann"
            if (nline + 1) % 4000 == 0:
                self.expe.log.info("processed {} lines".format(nline + 1))
            if line:
                json_data = line  # 载入这条数据，keys有Transcript # 改为document了
                src_ids = []
                src_eot = []
                src_entity_pos = []
                src_ent_set = set()
                src_n_ent = 0
                src_n_entry.append(len(json_data["document"]))
                for trans in json_data["document"]:  # json_data["document"]是list,句子的list
                    src_per_len_ = 0
                    for w in trans.split()[:max_src_len]:  # 3类，entity,vocab中的词，unk
                        src_tokens += 1
                        src_per_len_ += 1
                        src_eot.append(0)
                        if "ENTITY" in w:
                ...
                tgt_entity_pos = []
                tgt_n_ent = 0
                for w in " ".join(ann[nline]["summary"]).split()[:max_tgt_len]:
                    tgt_tokens += 1
                    if "ENTITY" in w:
                        tgt_ent_set.add(w)
                        tgt_n_ent += 1
                ...
                if src_ids and tgt_ids:
                    ...
                    if is_test:
                        all_instance[-1]["tgt_tok"] = \
                            " ".join(ann[nline]["summary"]).replace("@@ ", "")
                else:
                    self.expe.log.info(
                        "EMTPY FILE: {}".format(json_data["id"]))
        ...
        self.expe.log.info("#unique anomynized entity - src total: {} (avg: {:.2f}) | tgt total: {} (avg: {:.2f})"
                           .format(sum(src_unique_ent),
                                   sum(src_unique_ent) / len(src_unique_ent) if src_unique_ent else 0.0,
                                   sum(tgt_unique_ent),
                                   sum(tgt_unique_ent) / len(tgt_unique_ent) if tgt_unique_ent else 0.0))
...
class Minibatcher:
    ...
    def load(self, filename="minibatcher.ckpt"):
        if not filename:
            self.log.info("no minibatcher ckpt need to load")
            return
    ...
    def _reset(self):
        ...
        if self.is_eval:
            idx_list = idx_list[np.argsort([len(d["src_ids"]) for d in self.data])]  # 按长度排序
        else:
    ...
    def _pad_seq2seq_lf(self, data):
        ...
        max_tgt_len = max([len(d["tgt_ids"]) for d in data])
        max_eot = max([sum(d["src_eot"]) for d in data])  if not self.pad else self.pad[1] # 一个batch里面最多多少句句子
        max_pad_eot = max([max_eot - sum(d["src_eot"]) for d in data])


        if not self.pad:
            max_input_len = max_src_len + max_pad_eot
        else:
            max_input_len = sum(self.pad)

        input_data = \
            np.zeros((len(data), max_input_len)).astype("float32")
        input_attn_mask = \
            np.zeros((len(data), max_input_len)).astype("float32")
        global_attn_mask = \
            np.zeros((len(data), max_input_len)).astype("float32")
        ...
        for i, d in enumerate(data):
            ...
            input_data[i, :len(d["src_ids"])] = \
                np.asarray(list(proc_src_ids)).astype("float32")  # 把input的token id复制到0tensor中
            input_attn_mask[i, :len(d["src_ids"])] = 1.  # input长度的地方都赋值为1
            ...
            tgt_inp[i, :len(d["tgt_ids"]) + 1] = \
                np.asarray([self.vocab["<bos>"]] + proc_tgt_ids).astype("float32")  # 加一个bos开头
            tgt_tgt[i, :len(d["tgt_ids"]) + 1] = \
                np.asarray(proc_tgt_ids + [self.vocab["<eos>"]]).astype("float32")  # tgt_tgt和tgt_inp要错开一位，加一个eos
            tgt_mask[i, :len(d["tgt_ids"]) + 1] = 1.0  # 句子长度位置mask

            global_attn_mask[i, :len(d["src_eot"])] = d["src_eot"]  # 句子结尾处都赋值为1
            eot_idx[i, :len(d["src_eot"])] = d["src_eot"]
            eot_idx[i,
            len(d["src_eot"]): len(d["src_eot"]) + max_eot - sum(d["src_eot"])] = 1  # 把所有的句子长度补到一样，不然后面取出来不好计算
            eot_mask[i, :sum(d["src_eot"])] = 1  # 所有句子的EOS取出来之后的mask
        # for k in range(len(eot_idx)):
        #     print(k,sum(eot_idx[k,:]))

        return [input_data, input_attn_mask, global_attn_mask, tgt_inp, tgt_mask,
                tgt_tgt, eot_idx, eot_mask, [d["idx"] for d in data]]  # d[idx]是每条数据多少句子
